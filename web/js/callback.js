...
var versionCallback = 'Callback.js version: 12.6.1@20112016';
...
$(document).ready(function(){
    ...
    $('.container').on('click','.yMapsActive',function(){
        ...
        jsonAddressList = $('.unique-products-input-for-get-store-params').data('unique-json-list');
...
//    window.GLOBAL = '';
    ...
    //console.log('START');
    $("#windows .button_load").css('display','block');
...
//        console.log(item.product_id);
        ...
        ymaps.route([
            ...
            .then(function (route)
            {
                ...
                //console.log(item.address+' '+item.product_id+' '+length);
                //console.log('rstart');
                //console.log(rstart);
                //console.log('rend');
                //console.log(rend);
                ...
                var response = [key, item.product_id, length.toPrecision(3), item.address, item.id];
    ...
    /* -- last edited by Zloradnij -- */
    function allDone () {

        console.warn(minimumArr);

        $("#windows .button_load").hide();
        //  console.warn('END OF INFORMATION OUTPUT');
        addrCnt++;
        ...
        for(var x in newArray){
            //console.log('x = '+ x);
            //console.log('x = '+ newArray[x][1]);
            //console.log('count = '+basket.page.find('.all-products-input-for-set-store-params').find('input[data-shop-id="'+newArray[x][1]+'"]').length);
            basket.page.find('.all-products-input-for-set-store-params').find('input[data-shop-id="'+newArray[x][1]+'"]').val(newArray[x][4]);

//            $(' input[data-product-id='+x+']').siblings('input').val(newArray[x][4]);
    ...
    var Routing = chain(function () {
        ...
        for(var key in strClientAddress){
            for(var i in strClientAddress[key]) {
                //console.log(strClientAddress[key][i].product_id);
                sort(strClientAddress[key][i], strClientAddress[key][i].product_id);
            }
        }

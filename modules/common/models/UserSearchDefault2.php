...
/**
 * UserSearchDefault2 represents the model behind the search form about `app\modules\common\models\User`.
 */
class UserSearchDefault2 extends User
{
    public $orderDate;
    ...
    public function search($params)
    {
        ...
        $query = User::find()
            ...
            ->where('orders.id IN (
                ...
                AND (`orders_items`.`store_id` in (10000001,10000002,10000003,10000004,10000005,10000006,10000007,10000108,10000191))
...
WHERE DATE(`orders`.`date`) >= '2017-01-31' AND DATE(`orders`.`date`) <= '2017-02-09'
    ...
    AND (`orders_items`.`store_id` NOT IN (10000191, 10000001, 10000002, 10000003, 10000004, 10000005, 10000006, 10000007, 10000108))
      ...
      GROUP BY `order_id`, users.id;*/
        ...
        if ((isset($params['UserSearchDefault2']['dateStart'])) && (strtotime($params['UserSearchDefault2']['dateStart']) > 0) && (isset($params['UserSearchDefault2']['dateEnd'])) && (strtotime($params['UserSearchDefault2']['dateEnd']) > 0)) {
            $query->andFilterWhere(['between', 'orders.date', date('Y-m-d 00:00:00', (strtotime($params['UserSearchDefault2']['dateStart']))), date('Y-m-d 23:59:59', strtotime($params['UserSearchDefault2']['dateEnd']))]);
        }

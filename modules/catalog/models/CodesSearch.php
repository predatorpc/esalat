...
class CodesSearch extends Codes
{
    ...
    {
        return [
            [['id', 'type_id', 'user_id', 'count', 'status'], 'integer'],
            [['code', 'key', 'date_begin', 'date_end','summa','dateStart','dateStop', 'user_name'], 'safe'],
        ];
    }
...
//              'SUM(`users_pays`.`money` * (-1)) AS summa',
            ...
            ->leftJoin('orders','orders.code_id = codes.id')
//	    ->leftJoin('orders','orders.user_id = users_pays.user_id')
            ->leftJoin('users_pays','users_pays.order_id = orders.id')
//	    ->leftJoin('users','users.id = users_pays.user_id')
	...
	if(!empty($params['CodesSearch']['code'])){
//	    $query->andWhere('orders.code_id = '.$code->id);
	      $query->andWhere('codes.code = '.$params['CodesSearch']['code']);
	}
        ...
        if(!empty($params['CodesSearch']['user_id']) && intval($params['CodesSearch']['user_id'])!=0)
            $query->andWhere('codes.code = '.$params['CodesSearch']['user_id']);

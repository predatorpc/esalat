 ...
 *
 */
class WNewProductItem extends Widget {

    public function run(){
         $newProduct = Goods::find()
             ->where(['goods.status' => 1, 'goods.show' => 1, 'goods.confirm' => 1, 'goods.new' => 1, ])->orderBy('goods.id  DESC')->limit(20)->all();
        ...
        ?>
            <div class="items">
                ...
                <div id="<?=$product->id?>" class="item item-<?= $product->id?>">
                    <div class="block">
                        <div class="images">
                            <a href="<?=$url?>"><?php if(!empty($product->imageSimple)): ?><img alt="<?=$product->name?>" src="http://www.esalad.ru/<?=$product->imageSimple;?>" class="ad"><?php else: ?><img src="/images/<?=(!empty(\Yii::$app->params['en']) ? 'good_min_en' : 'good_min')?>.png" alt="<?=$product->name?>" class="ad"/><?php endif;?></a>
                        </div>

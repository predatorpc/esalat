...
class WWishlistProductItemOne extends Widget {
    public $model;
    ...
    public function run(){
        if(!$this->model){
            ...
            $cat = Category::find()->where(['parent_id' =>  $this->categoryCurrent['id']])->count('id');
                ...
                <div id="<?= $this->model->id?>" data-position="<?=$this->model->position?>"   data-product-id="<?= $this->model->id?>" class="item   <?=($this->model->type == 1 ? 'max':'')?> item-<?= $this->model->id?>  product-item js-product-item" >
                    <div class="block <?= (!empty($color) ? 'ef': '')?>"      <?php if(!empty($color)): ?>style="border-bottom-color:<?=strlen($color)<7?'#'.$color:$color ?>;" <?php endif; ?>    >
                        <div class="images" onclick="return show_modal_compact('/catalog/compact',' ','<?=$this->model->id?>');" >
                            ...
                            <div class="compact" onclick="return show_modal_compact('<?= Url::toRoute('/'.$this->model->category->catalogPath . $this->model->id) ?>',' ');"><div>Быстрый просмотр</div></div><!--.Быстрый просмотр-->
                        </div>
                        <!--block max-->
                        <div class="block-t">
                            <div class="rating">
                        ...
                        <div class="clear"></div>
                        <div class="title" onclick="return show_modal_compact('/catalog/compact/<?=$this->model->id?>',' ');" >
                                    ...
                                    if($this->model->discount == 1 && !$this->model->discount){?>
                                        ...
                                        <span class="price normal variation-price">
                                             ...
                                             <?php if(!Yii::$app->user->isGuest){
                                                 echo '<span class="text-danger" style="font-size: 11px;">'.round(Yii::$app->user->identity->bonus/($allVariation->priceValue/100),1).'%</span>';
                                             } ?>
                                          </span>
                                    <?php }?>
                                    </div><?php
                                    ...
                                    if(isset($allVariations) && !empty($allVariations)){
                                        foreach($allVariations as $variantKey => $variantItem){
                                            ...
                                            >
                                                ...
                                                <?php if(!Yii::$app->user->isGuest){
                                                    $wishlistProducts = WishlistProducts::find()->where(['user_id'=>Yii::$app->user->id,'product_id'=>$this->model->id,'status'=>1])->one();

                                                    if(Url::current() == '/my/wish-list'){
                                                        ;?>
                                                <div
                                                    class="center remFromWishlist_btn glyphicon glyphicon-remove-sign _icon_button remove"
                                                    data-action=""
                                                    title="Удалить"
                                                    data-basket=""
                                                ...
                                                >
                                                        ...
                                                        <?php if(round(Yii::$app->user->identity->bonus/($allVariation->priceValue/100),1)>= 30){?>
                                                            <div
                                                                    class="button-basket-icon center basket_button glyphicon _icon_button <?=(!empty($wishlistProducts) ? 'glyphicon-ok-sign success' : 'glyphicon-plus-sign oran')?> "
                                                                    data-action="bay"
                                                                    data-basket=""
                                                                    data-product="<?= $this->model->id?>"
                                                                    data-variant="<?= $variantItem->id?>"
                                                        ...
                                                        <div
                                                                class="center wishlist_btn glyphicon _icon_button <?=(!empty($wishlistProducts) ? 'glyphicon-ok-sign success' : 'glyphicon-plus-sign oran')?>"
                                                                data-action=""
                                                                data-basket=""
                                                                data-product="<?= $this->model->id?>"
                                                                data-variant="<?= $variantItem->id?>"
                        ...
                        <!-- Управление -->
                        <?php if(!empty(\Yii::$app->user->identity) && (\Yii::$app->user->can('categoryManager') || \Yii::$app->user->can('conflictManager') || \Yii::$app->user->can('GodMode'))): ?>
